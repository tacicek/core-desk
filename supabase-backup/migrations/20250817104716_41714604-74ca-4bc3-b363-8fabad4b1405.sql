...
-- Add trigger to log customer modifications for audit purposes
...
AS $$
BEGIN
  -- Log customer data changes for audit purposes
...
-- Create trigger for audit logging (only for modifications, not SELECT)
DROP TRIGGER IF EXISTS customers_audit_trigger ON public.customers;
CREATE TRIGGER customers_audit_trigger
  AFTER INSERT, UPDATE, DELETE ON public.customers
  FOR EACH ROW
