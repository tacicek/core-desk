-- Fix the search_path security issue in the functions
DROP FUNCTION IF EXISTS public.get_annual_summary();
DROP FUNCTION IF EXISTS public.get_tax_report();

-- Create a secure function for annual summary data with proper search_path
...
AS $$
  ...
  WHERE be.expense_date IS NOT NULL
    AND be.vendor_id = public.get_user_vendor_id()  -- Enforce vendor access
  GROUP BY be.vendor_id, EXTRACT(year FROM be.expense_date), be.tax_category;
$$;

-- Create a secure function for tax report data with proper search_path
